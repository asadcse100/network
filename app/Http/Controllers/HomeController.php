...
class HomeController extends Controller
{
    ...
    if ($v['type'] == 'VPN') {
      ...
      if ($pu->vpn_clicks >= env('VPN_CLICK_LIMIT')) {
        ...
        $data = array(
          'name' => $pu->name,
          'publisher_id' => $pu->id,
          'message' => 'Your Account has been Banned Due to Exceeded Vpn Clicks',
          'subject' => 'Account Ban',
          'email' => $pu->email
        );
        ...
        if (!empty($smtp_server->smtp_host) && !empty($smtp_server->smtp_port) && !empty($smtp_server->smtp_user) && !empty($smtp_server->smtp_password)) {
  ...
  public function subscriber_store(Request $request)
  {
    ...
    return redirect()->back()->with('success','Message send successful. Soon comunicate with you.');
  }

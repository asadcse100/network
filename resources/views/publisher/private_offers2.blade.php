...
<div class="page-content-wrapper">
      ...
      <div class="col-lg-12 ">
        <div class="card">
          <div class="card-header">
            <h5 class="mb-0">Filter Private Offers</h5>
          </div>
          <div class="card-body">
            <form id="form1">
              @csrf
              <div class="row">
                <div class="col-lg-12 mb-3">
                  ...
                  <div slider id="slider-distance " style="margin: 0px!important;margin-bottom: 20px!important;">
                    ...
                    <input type="range" tabindex="0" value="30" max="100" min="0" name="from_payout" step="1" oninput="
  ...
  children[11].childNodes[1].innerHTML=this.value;" />
                    <input type="range" tabindex="0" value="60" max="100" min="0" name="to_payout" step="1" oninput="
  ...
  children[13].childNodes[1].innerHTML=this.value;" />
                ...
                <div class="col-lg-3">
                  ...
                  <select class="js-example-basic-multiple" name="ua_target[]" id="ua_target" multiple data-actions-box="true" style="width: 100%">
                ...
                <div class="col-lg-4">
                  ...
                  <select class="js-example-basic-multiple" name="category[]" id="category" multiple data-actions-box="true" style="width: 100%">
                    @foreach (DB::table('category')->where('is_deleted', '0')->get() as $q)
            ...
            <div class="table-responsive">
              <table id="example2" class="table table-bordered table-sm table-striped w-100">
                ...
                <tbody id="showdata">
                  ...
                  $qry = DB::select("SELECT o.offer_name,ap.approval_status,o.offer_type,c.category_name,o.countries,o.payout_percentage,o.payout_type,o.payout,o.ua_target,o.status,o.clicks,o.conversion,o.browsers,o.incentive_allowed,o.smartlink,o.magiclink,o.native,o.lockers,o.preview_url,o.verticals,o.id as oid,(select GROUP_CONCAT(pub.name) from offers_publisher as of join publishers as pub on pub.id=of.publisher_id where of.offer_id=o.id) as publisher_name  FROM `offers`  as o left join approval_request as ap on ap.offer_id=o.id and ap.publisher_id='$id'   left join category as c on c.id=o.category_id where  o.magiclink!=1 and o.smartlink!=1 and o.native!=1 and o.lockers!=1   and o.status='Active' and o.offer_type='private'  order by o.id desc");
                  $site = DB::table('site_settings')->first();
                  foreach ($qry as $q) { ?>
                    <tr>
                      <td>{{$q->oid}}</td>
                      <td>@if(!empty($q->preview_url))<a href="{{asset('uploads/')}}/{{$q->preview_url}}" target="_blank"><img src="{{asset('uploads/')}}/{{$q->preview_url}}" width="100 " height="100" style="object-fit: contain;"></a>@endif</td>
                      <td>{{$q->offer_name}}</td>
                      <td>{{$q->category_name}}<br>{{$q->verticals}}</td>
                      <td> <?php if ($q->payout_type == 'revshare') { ?>
                          RevShare
                        <?php } else {
                        ?>
                      ...
                      <td><?php if ($q->approval_status == 'Approved') { ?>

                          <a class="btn btn-primary" href="{{url('publisher/offers-details')}}/{{$q->oid}}" target="_blank">Get Details</a>
                        <?php } else if ($q->approval_status == 'Requested') {
                        ?>
                          <a class="btn btn-warning" href="{{url('publisher/offers-details')}}/{{$q->oid}}" target="_blank"> Approval Requested</a>
                        <?php
                          } else if ($q->approval_status == 'Rejected') {

                        ?>
                          <a class="btn btn-danger" href="{{url('publisher/offers-details')}}/{{$q->oid}}" target="_blank">Approval Rejected</a>
                        <?php
                          } else {
                        ?>
                          <a class="btn btn-success" href="{{url('publisher/offers-details')}}/{{$q->oid}}" target="_blank">Request Approval</a>
  ...
  $(function() {
    $('#search').click(function() {
      ...
      $.ajax({
        method: 'get',
        data: data,
        url: '<?php echo url('publisher/search-private-offer') ?>',
        async: false,
        dataType: 'json',
        success: function(res) {
          ...
          for (var i = 0; i < res.length; i++) {
            ...
            if (res[i].approval_status == 'Approved') {
              button = '<a   class="btn btn-primary" href="{{url('
              publisher / offers - details ')}}/' + res[i].offerid + '" target="_blank">Get Details</a>';
            } else if (res[i].approval_status == 'Rejected') {
              button = '<a   class="btn btn-danger" href="{{url('
              publisher / offers - details ')}}/' + res[i].offerid + '" target="_blank">Approval Rejected</a>';
            } else if (res[i].approval_status == 'Requested') {
              button = '<a   class="btn btn-primary" href="{{url('
              publisher / offers - details ')}}/' + res[i].offerid + '" target="_blank">Approval Requested Rejected</a>';
            } else {
              button = '<a  class="btn btn-primary"  href="{{url('
              publisher / offers - details ')}}/' + res[i].offerid + '" target="_blank">Request Approval</a>';
            }

...
$qry = DB::table('offers as o')->select('*', 'o.id as oid')->leftjoin('category as c', 'c.id', '=', 'o.category_id')->where('o.id', $data)->where('o.status', 'Active')->first();
?>
			...
			<div class="col-lg-12 ">
				<div class="card">
					<div class="card-header">
						<h5 class="mb-0">OFFER ID : <span class="text-danger">@if(!empty($qry->oid)){{$qry->oid}}@endif</span></h5>
					</div>
					<div class="card-body">
						<h6>Offer Details of <b style="color:#800000">@if(!empty($qry->offer_name)){{$qry->offer_name}}@endif</b></h6>
						@if(!empty($qry->offer_type))
						...
						if ($count > 0) { ?>
							...
							@if($qry->preview_link!='')
							<a href=" {{$qry->preview_link}}" class="btn btn-success" target='blank'>Preview Link</a>
							@endif
							<div class="mt-3">
							...
							<div class="mt-5 mb-5">
								...
								<?php if ($qry->smartlink != 1) { ?>
									...
									<p class="text-danger" id="offer_name1">If you want to use Sub Affiliate then enter the value. Otherwise do not enter anything.You can use one or one more sub id</p>
									<table class="table">
										<thead>
											<th>Enter SubID1</th>
											<th>Enter SubID2</th>
	...
	$(function() {
		...
		$('#btnUpdate').click(function() {
			$.ajax({
				...
				url: "{{url('publisher/requestApproval')}}",

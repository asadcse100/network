      ...
      <div class="col-lg-12 ">
        <div class="card">
          <div class="card-header">
            <h5 class="mb-0">Filter New Offers</h5>
          </div>
          <div class="card-body">
            <form action="{{url('publisher/new_offer/search')}}" method="get">
              @csrf
              <div class="row">
              ...
              <input type="hidden" name="offer_type" value="new">
            </form>
          </div>
        </div>
        ...
        <div class="card">
          <div class="card-header">
            <h5 class="mb-0">View New Offers</h5>
          </div>
          <div class="card-body">
              ...
              <table id="example2" class="table table-bordered table-sm table-striped w-100">
                ...
                <tbody id="showdata">
                  ...
                  foreach ($offers as $offer) { ?>
                    <tr>
                      <td>{{$offer->id}}</td>
                      <td>@if(!empty($offer->preview_url))<a href="{{asset('uploads/')}}/{{$offer->preview_url}}" target="_blank"><img src="{{asset('uploads/')}}/{{$offer->preview_url}}" width="100 " height="100" style="object-fit: contain;"></a>@endif</td>
                      ...
                      <td>
                        <a class="btn btn-primary" href="{{url('publisher/offers-details')}}/{{$offer->id}}" target="_blank">Get Details</a>
                      </td>
                    </tr>
                  <?php }
                  ?>
  ...
  $(function() {
    $('#search').click(function() {
      ...
      $.ajax({
        method: 'get',
        data: data,
        url: '<?php echo url('publisher/search-new-offer') ?>',
        async: false,
        dataType: 'json',
        success: function(res) {
          ...
          for (var i = 0; i < res.length; i++) {
            sno++;
            var button = '';
            if (res[i].magiclink == 0 || res[i].magiclink == '' && res[i].smartlink == 0 || res[i].smartlink == '' && res[i].lockers == 0 || res[i].lockers == '' && res[i].native == 0 || res[i].native == '') {
              if (res[i].offer_type == 'private') {
                if (res[i].approval_status == 'Approved') {
                  button = '<a  class="btn btn-primary" href="{{url("publisher/offers-details")}}/' + res[i].offerid + '" target="_blank">Get Details</a>';
                } else if (res[i].approval_status == 'Rejected') {
                  button = '<a  class="btn btn-danger" href="{{url("publisher/offers-details")}}/' + res[i].offerid + '" target="_blank">Approval Rejected</a>';
                } else if (res[i].approval_status == 'Requested') {
                  button = '<a  class="btn btn-primary" href="{{url("publisher/offers-details")}}/' + res[i].offerid + '" target="_blank">Approval Requested Rejected</a>';
                } else {
                  button = '<a   class="btn btn-primary" href="{{url("publisher/offers-details")}}/' + res[i].offerid + '" target="_blank">Request Approval</a>';
                }
              } else {
                button = '<a  class="btn btn-primary" href="{{url("publisher/offers-details")}}/' + res[i].offerid + '" target="_blank">Get Details</a>';
              }

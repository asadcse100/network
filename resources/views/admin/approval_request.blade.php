...
<script type="text/javascript">
	...
	$(function() {
		...
		function showdata() {
			$.ajax({
				method: 'get',
				url: '<?php echo url('admin/show-approval-request') ?>',
				async: false,
				dataType: 'json',
		...
		$('#showdata').on('click', '.approveData', function() {
			...
			if (r == true) {
				window.location.href = '<?php echo url('admin/approve-request') ?>/' + id + '';
			}
		})
		//EDIT DATA
		$('#showdata').on('click', '.rejectData', function() {
			...
			if (r == true) {
				window.location.href = '<?php echo url('admin/reject-request') ?>/' + id + '';
			}
		})

						...
						<div class="row">
							...
							<div class="col-lg-6 col-sm-6">
								<div class="text-right">
									<button class="btn btn-outline-primary" data-toggle="modal" data-target="#addModal">Add Categories</button>
								</div>
							</div>
						</div>
					</div>
			...
			$(function() {
				...
				$('#showdata').on('click', '.editData', function() {
					...
					$.ajax({
						...
						url: '<?php echo url('admin/edit-categories') ?>',
						async: false,
						dataType: 'json',
						success: function(res) {
							$('input[name=Categories_name1]').val(res.category_name);
							$('#id').val(res.id);
				...
				$('#showdata').on('click', '.deleteData', function() {
					...
					$('#btnDelete').unbind().click(function() {
						$.ajax({
							...
							url: '<?php echo url('admin/delete-categories') ?>',
							async: false,
							dataType: 'json',
							success: function(res) {
				...
				$('#btnUpdate').unbind().click(function() {
					...
					$.ajax({
						method: 'post',
						data: data,
						url: '<?php echo url('admin/update-categories') ?>',
						async: false,
						dataType: 'json',
						success: function(res) {
							$('#form2')[0].reset();
				...
				$('#btnSave').unbind().click(function() {
					...
					if (result == '1') {
						$.ajax({
							method: 'post',
							data: data,
							url: '<?php echo url('admin/insert-categories') ?>',
							async: false,
							dataType: 'json',
							success: function(res) {
								$('#form1')[0].reset();

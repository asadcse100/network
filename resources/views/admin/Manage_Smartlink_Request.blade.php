...
<script type="text/javascript">
	...
	$(function() {
		...
		function showdata() {
			$.ajax({
				method: 'get',
				url: '<?php echo url('admin/show-smartlink-request') ?>',
				async: false,
				dataType: 'json',
				success: function(res) {
					...
					for (var i = 0; i < res.length; i++) {
						...
						html += '<tr>' +
							...
							'<td>' + parseFloat(res[i].earnings).toFixed(3) + '</td>' +
		...
		$('#showdata').on('click', '.approveData', function() {
			...
			if (r == true) {
				window.location.href = '<?php echo url('admin/smartlink-approve-request') ?>/' + id + '';
			}
		})
		//EDIT DATA
		$('#showdata').on('click', '.rejectData', function() {
			...
			if (r == true) {
				window.location.href = '<?php echo url('admin/smartlink-reject-request') ?>/' + id + '';
			}
		})

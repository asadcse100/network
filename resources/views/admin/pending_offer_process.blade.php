                  ...
                  <form id="form">
                    ...
                    <tbody class="text-dark">
                      ...
                      <tr>
                        ...
                        @php
                        $publisher = DB::table('publishers')->where('id', $q->publisher_id)
                        ->first();
                        $advertiser = DB::table('advertisers')->where('id', $q->advertiser_id)
                        ->first();
                        @endphp
                        <td>{{$publisher->name}}</td>
                        <td>{{$publisher->email}}</td>
  ...
  $(function() {
    ...
    function searchData() {
      ...
      $.ajax({
        method: 'get',
        data: data,
        url: '<?php echo url('admin/search-pending-offer-process') ?>',
        async: false,
        dataType: 'json',
        success: function(res) {
          let html = '';
          ...
          for (var i = 0; i < res.length; i++) {
            ...
            html += '<tr>' +
              ...
              '<td>' + parseFloat(res[i].payout, 3).toFixed(3) + '</td>' +
